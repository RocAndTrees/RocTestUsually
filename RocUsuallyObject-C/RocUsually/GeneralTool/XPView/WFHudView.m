...
-(id)initWithMsg:(NSString*)_msg
{
    if (self = [super init]) {
        ...
        if ([self getSizeFromString:_msg].height > 32) {
            ...
            //改变view的高度
//            self.bounds = CGRectMake(0, 0, textSize.width + (160 - textSize.width) / 2, textSize.height + 8);
        }
...
-(CGSize)getSizeFromString:(NSString*)_theString
{
   ...
   CGSize tempSize = [_theString boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:theFont} context:nil].size;
