...
static const char * const kZSYPopoverListButtonClickForRightButton = "kZSYPopoverListButtonClickForRightButton";
...
@synthesize titleRightButton = _titleRightButton;
@synthesize titleLeftButton = _titleLeftButton;
...
//    self.layer.cornerRadius = 10.0f;
    ...
    self.titleName.backgroundColor =  [UIColor colorWithRed:60/255.
                                                      green:60/255.
                                                       blue:60/255.
                                                      alpha:1.0f];
    ...
    [self.mainPopoverListView setTableFooterView:[[UIView alloc] init]];
...
- (void)show
{
    ...
    self.center = CGPointMake(keywindow.bounds.size.width/2.0f,
                              keywindow.bounds.size.height/2.0f + (keywindow.bounds.size.height -250)/2);
    [self animatedIn];
}
...
- (void)setTitleRightButtonTitle:(NSString *)aTitle block:(ZSYPopoverListViewButtonBlock)block
{
    if (nil == _titleRightButton)
    {
        _titleLeftButton = [[UIButton alloc] initWithFrame:CGRectZero];
        CGFloat xWidth = self.bounds.size.width;
        self.titleLeftButton.frame = CGRectMake(8, 0, 32, 32);
        [self.titleLeftButton setImage:[UIImage imageNamed:@"xc_wh"] forState:UIControlStateNormal];
        [self.titleLeftButton addTarget:self action:@selector(buttonWasPressed:) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.titleLeftButton];
        
        
        _titleRightButton = [[UIButton alloc] initWithFrame:CGRectZero];
        self.titleRightButton.frame = CGRectMake(xWidth-68, 0, 60, 32);
        [self.titleRightButton setTitle:@"确定" forState:UIControlStateNormal];
        [self.titleRightButton addTarget:self action:@selector(animatedOut) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview:self.titleRightButton];

    }
    objc_setAssociatedObject(self.titleLeftButton, kZSYPopoverListButtonClickForRightButton, [block copy], OBJC_ASSOCIATION_RETAIN);
}
...
- (void)buttonWasPressed:(id)sender
{
    ...
    {
        block = objc_getAssociatedObject(sender, kZSYPopoverListButtonClickForRightButton);
    }
    if (block)
    {
        block();
